...
my @duporg;
my %orgacc;
open(my $refh, "<", "all_refrep_withcds.list");
while(my $line = readline($refh)) {
chomp($line);
if($line=~m/^\s*\#/ or $line=~m/^\s*$/) {next;}
my ($acc, $org, $lin) = split(/\t/, $line);
if(exists($orgacc{$org})) {
  unless( grep {$_ eq $org} @duporg ) {
    push(@duporg, $org);
    #carp("$acc --- $org");
  }
}
