...
tablist("Accession", "Organism", "LocusTag", "ProteinLength",  "Identity", "E-value",
    "AnnotatedProduct", "Lineage");

my @sorted = sort {$a->[-1] cmp $b->[-1]} @lol;
my %accessions;
for my $lr (@sorted) {
  $accessions{$lr->[0]} += 1;
  tablist(@{$lr});
...
my @uniqAcc = keys(%accessions);
linelistE("Unique Accessions: ", scalar(@uniqAcc)); 
