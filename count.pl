...
my %spcnt; # Species counts.
my %gencnt; # Genus counts.
my %accnt; # lists of accessions for species.
my %gaccnt; # lists of accessions for genera.
my %lineage;
...
my $acc = $ll[0];
my $org = $ll[1];
my @orgl = split(/\s+/, $org, 3);
my $species = $orgl[0] . " " . $orgl[1];
my $genus = $orgl[0];
# tablistE($org, $species);
$spcnt{$species} += 1;
$gencnt{$genus} += 1;
$lineage{$species} = $ll[7];
$lineage{$genus} = $ll[7];


unless(grep { $_ eq $acc } @{$accnt{$species}}) {
  push(@{$accnt{$species}}, $acc);
}
unless(grep { $_ eq $acc } @{$gaccnt{$genus}}) {
  push(@{$gaccnt{$genus}}, $acc);
}
...
tablist(qw(Serial Species NumAcc NumHits Lineage));

my @species = keys(%spcnt);
my @sorted = sort @species;
my $serial = 0;
for my $sp (@sorted) {
  $serial += 1;
  my $acnt = scalar(@{$accnt{$sp}});
  tablist($serial, $sp, $acnt, $spcnt{$sp}, $lineage{$sp});
}
tablist(qw(Serial Genus NumAcc NumHits Lineage));
my @genera = keys(%gencnt);
@sorted = sort @genera;
$serial = 0;
for my $genus (@sorted) {
  $serial += 1;
  my $acnt = scalar(@{$gaccnt{$genus}});
  tablist($serial, $genus, $acnt, $gencnt{$genus}, $lineage{$genus});
}
